...
// }
void allocate_memory(list_t *freelist, list_t *alloclist, int pid, int blocksize, int policy) {
    // Step 1: Check if a node is in the FREE_LIST with a blk(end - start) >= blocksize
    ...
    while (current != NULL) {
        ...
        if ((blk->end - blk->start + 1) >= blocksize) {
            ...
            allocated_block = list_remove_at_index(freelist, index);
            break;
        }
    ...
    // Step 2: If no suitable block was found, print an error and return
    if (allocated_block == NULL) {
        fprintf(stderr, "Error: Memory Allocation %d blocks\n", blocksize);
        return;
    }

    // Step 3: Set the blk.pid = pid
    allocated_block->pid = pid;

    // Step 4: Set the blk.end = blk->start + blocksize - 1
    int temp_end = allocated_block->end;
    ...
    // Step 6a: Dynamically allocate a new block_t called fragment [use malloc]
    block_t *fragment = (block_t *)malloc(sizeof(block_t));

    // Step 6b: Set the fragment->pid = 0
    fragment->pid = 0;

    // Step 6c: Set the fragment->start = the blk.end + 1
    fragment->start = allocated_block->end + 1;

    // Step 6d: Set the fragment->end = original blk.end before you changed it in #4
    fragment->end = temp_end;

    // Step 6e: Add the fragment to the FREE_LIST based on policy
    if (policy == 1) {
        // FIFO policy (add to the front)
        list_add_to_back(freelist, fragment);
    } else if (policy == 2) {
        // BESTFIT or WORSTFIT policy (add in ascending order by size)
...
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy) {
    ...
    block_t *deallocated_block = NULL;
    if (list_is_in_by_pid(alloclist, pid)){
        int index = list_get_index_of_by_Pid(alloclist, pid);
        deallocated_block = list_remove_at_index(alloclist, index);
    }
    // Step 2: If no block with the specified PID was found, print an error and return
    else{
        fprintf(stderr, "Error: Can't locate Memory Used by PID: %d\n", pid);
        return;
    }

    // Step 3: Set the blk.pid back to 0
